        ...
        <table class="data-table">
          ...
          <tbody>
            <tr v-for="record in filteredRecords" :key="record.recordId">
              ...
              <td>{{ record.staff?.fullName || 'N/A' }}</td>
...
const search = () => {
  ...
  filteredRecords.value = medicalRecords.value.filter(record => 
    (record.patient?.fullName?.toLowerCase().includes(lowerCaseSearchTerm)) ||
    (record.staff?.fullName?.toLowerCase().includes(lowerCaseSearchTerm)) ||
